  ...
  db.query(
    ...
    (err, deleteResults) => {
      if (err) {
        console.error(err);
        return res.send(
          '<script>alert("Customer Associated with rentals"); window.location="/customers";</script>'
        );
      }

      // Redirect with success message
      return res.send(
        '<script>alert("Customer deleted successfully."); window.location="/customers";</script>'
      );
    }
  );
});
